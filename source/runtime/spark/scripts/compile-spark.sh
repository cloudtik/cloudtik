...
if [ $APPLY_PATCH ]; then
    if [ -d "$CLOUDTIK_HOME/source/runtime/spark/spark-${SPARK_VERSION}" ]; then
        for patch in $CLOUDTIK_HOME/source/runtime/spark/spark-${SPARK_VERSION}/optimizations/*.patch; do
            git apply $patch
        done
    fi
fi
...
mkdir -p $CLOUDTIK_HOME/source/runtime/spark/dist
cp -r /tmp/spark/spark-${SPARK_VERSION}-bin-hadoop3.tgz $CLOUDTIK_HOME/source/runtime/spark/dist/
