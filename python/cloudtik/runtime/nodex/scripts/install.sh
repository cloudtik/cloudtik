...
install_nodex() {
    if [ ! -f "${NODEX_HOME}/nodex" ]; then
        ...
        (cd $RUNTIME_PATH && wget -q --show-progress https://github.com/prometheus/node_exporter/releases/download/v${NODEX_VERSION}/node_exporter-${NODEX_VERSION}.linux-${deb_arch}.tar.gz -O nodex.tar.gz && \
          mkdir -p "$NODEX_HOME" && \
