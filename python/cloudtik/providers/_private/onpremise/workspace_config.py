...
def get_workspace_head_nodes(
    ...
    all_heads = _get_http_response_from_simulator(
...
def list_onpremise_clusters(
    ...
    head_nodes = get_workspace_head_nodes(
        workspace_name, provider_config)
    clusters = {}
...
def _create_simulator_workspace(provider_config, workspace_name):
    if _is_workspace_exists(provider_config, workspace_name):
        cli_logger.print(
            "On-premise workspace already exists. Skip creation.")
        return

    try:
        cli_logger.print(
            "Creating on-premise workspace: {}.", workspace_name)
        ...
        cli_logger.print(
            "Successfully created on-premise workspace.")
    except Exception as e:
        cli_logger.error(
            "Failed to create on-premise workspace: {}", str(e))
        raise e
...
def _delete_simulator_workspace(provider_config, workspace_name):
    if not _is_workspace_exists(provider_config, workspace_name):
        cli_logger.print(
            "On-premise workspace doesn't exist. Skip deletion.")
        return

    try:
        cli_logger.print(
            "Deleting on-premise workspace: {}.", workspace_name)
        ...
        cli_logger.print(
            "Successfully deleted on-premise workspace.")
    except Exception as e:
        cli_logger.error(
            "Failed to delete on-premise workspace: {}", str(e))
        raise e
