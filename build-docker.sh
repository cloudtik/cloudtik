...
do
    ...
    *)
        ...
        echo "[ --build-ai ] [ --build-ai-oneapi ] [ --build-ai-benchmark ]"
        echo "[ --build-spark-ai ] [ --build-spark-ai-oneapi ] [ --build-spark-ai-benchmark ]"
...
if [ -z "$WHEEL_TO_USE" ]; then
    if [ "$IMAGE_TAG" == "nightly" ]; then
        WHEEL_URL="https://d30257nes7d4fq.cloudfront.net/downloads/cloudtik/cloudtik-${CLOUDTIK_VERSION}-cp${PYTHON_TAG}-cp${PYTHON_TAG}-manylinux2014_${arch}.nightly.whl"
    else
        WHEEL_URL="https://d30257nes7d4fq.cloudfront.net/downloads/cloudtik/cloudtik-${CLOUDTIK_VERSION}-cp${PYTHON_TAG}-cp${PYTHON_TAG}-manylinux2014_${arch}.whl"
    fi
else
...
if [ $BUILD_CLOUDTIK ] || [ $BUILD_ALL ]; then
    ...
    do
        cp "$WHEEL" "docker/$IMAGE/$WHEEL_NAME"
        if [ $OUTPUT_SHA ]; then
            IMAGE_SHA=$(docker build $NO_CACHE --build-arg DEVICE_TYPE="$DEVICE_TYPE" --build-arg BASE_IMAGE="$BASE_IMAGE" --build-arg WHEEL_PATH="$WHEEL_NAME" --build-arg PYTHON_VERSION="$PYTHON_VERSION" --build-arg CONDA_ENV_NAME="$CONDA_ENV_NAME" -q -t cloudtik/$IMAGE:$IMAGE_TAG$DEVICE_TAG docker/$IMAGE)
            echo "cloudtik/$IMAGE:$IMAGE_TAG$DEVICE_TAG SHA:$IMAGE_SHA"
        else
            docker build $NO_CACHE --build-arg DEVICE_TYPE="$DEVICE_TYPE" --build-arg BASE_IMAGE="$BASE_IMAGE" --build-arg WHEEL_PATH="$WHEEL_NAME" --build-arg PYTHON_VERSION="$PYTHON_VERSION" --build-arg CONDA_ENV_NAME="$CONDA_ENV_NAME" -t cloudtik/$IMAGE:$IMAGE_TAG$DEVICE_TAG docker/$IMAGE
        fi
        rm "docker/$IMAGE/$WHEEL_NAME"
    done
    ...
    do
        cp "$WHEEL" "docker/$IMAGE/$WHEEL_NAME"
        if [ $OUTPUT_SHA ]; then
            IMAGE_SHA=$(docker build $NO_CACHE --build-arg DEVICE_TYPE="$DEVICE_TYPE" --build-arg BASE_TAG=$IMAGE_TAG$DEVICE_TAG --build-arg WHEEL_PATH="$WHEEL_NAME" --build-arg PYTHON_VERSION="$PYTHON_VERSION" -q -t cloudtik/$IMAGE:$IMAGE_TAG$DEVICE_TAG docker/$IMAGE)
            echo "cloudtik/$IMAGE:$IMAGE_TAG$DEVICE_TAG SHA:$IMAGE_SHA"
        else
            docker build $NO_CACHE --build-arg DEVICE_TYPE="$DEVICE_TYPE" --build-arg BASE_TAG=$IMAGE_TAG$DEVICE_TAG --build-arg WHEEL_PATH="$WHEEL_NAME" --build-arg PYTHON_VERSION="$PYTHON_VERSION" -t cloudtik/$IMAGE:$IMAGE_TAG$DEVICE_TAG docker/$IMAGE
        fi
        rm "docker/$IMAGE/$WHEEL_NAME"
    done
fi
